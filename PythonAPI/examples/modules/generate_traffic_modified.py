...
class TrafficTelemetryPublisher(Telemetry):
    ...
    ACTOR_CACHE_INTERVAL = 1.0 # Refresh cached actor list every 1 second
    ...
    def handle_fetch_telemetry_data(self):
        now = time.monotonic()
        if now - self._last_cache_time >= self.ACTOR_CACHE_INTERVAL:
            with self._world_lock:
                self._cached_vehicles = list(self._world.get_actors().filter("vehicle.*"))
            self._last_cache_time = now

        messages = []
        for vehicle in self._cached_vehicles:
...
def main():
    ...
    should_stop_respawn = threading.Event()
    respawn_thread = None
    ...
    try:
        ...
        def respawn_monitor():
            nonlocal vehicles_list
            while not should_stop_respawn.is_set():
                should_stop_respawn.wait(RESPAWN_CHECK_INTERVAL)
                if should_stop_respawn.is_set():
                    break

                with vehicles_lock:
                    current_vehicles = list(vehicles_list)

                alive_actors = world.get_actors(current_vehicles)
                healthy_ids = set()
                remove_ids = []
                ...
                missing_ids = [vid for vid in current_vehicles if vid not in alive_actor_ids]
                if remove_ids:
                ...
                new_vehicles_list = [vid for vid in current_vehicles if vid in healthy_ids]
                vehicles_to_spawn = target_vehicle_count - len(new_vehicles_list)
                ...
                if vehicles_to_spawn > 0:
                    ...
                    respawned = len(new_vehicles_list) - (target_vehicle_count - vehicles_to_spawn)
                    if respawned > 0:
                        logging.info('Respawned %d vehicles (total: %d/%d)', respawned, len(new_vehicles_list), target_vehicle_count)

                with vehicles_lock:
                    vehicles_list = new_vehicles_list

        should_stop_respawn = threading.Event()
        respawn_thread = threading.Thread(target=respawn_monitor, daemon=True)
        ...
        while True:
            if not args.asynch and synchronous_master:
                ...
                remaining = settings.fixed_delta_seconds - tick_elapsed
                if remaining > 0:
        ...
        should_stop_respawn.set()
        if respawn_thread and respawn_thread.is_alive():
