...
class MultiAgentRenderer(Telemetry):
        ...
        for vehicle_message in parsed_message["vehicles"]:
            ...
            try:
                with self._lock:
                    ...
                    needs_spawn = vehicle_id not in self.vehicles
                    needs_reload = not needs_spawn and self._has_other_vehicle_changed(vehicle_message)
                    needs_update = not needs_spawn and not needs_reload and vehicle_id in self.vehicles
    ...
    def _cleanup_stale_vehicles(self):
        ...
        stale = [v for v in existing_vehicles
                 if v.attributes.get("role_name") not in ("hero", "ego_vehicle")]
        if stale:
            for vehicle in stale:
                vehicle.destroy()
            print(f"[!] Cleaned up {len(stale)} stale vehicles from previous runs")
    ...
    def _has_other_vehicle_changed(self, parsed_message):
        ...
        if vehicle.type_id != parsed_message["blueprint"]:
            return True

        vehicle_color = vehicle.attributes.get("color")
        if vehicle_color is not None and vehicle_color != parsed_message.get("color"):
            return True
    ...
    def _reload_other_vehicle(self, parsed_message, spawn_point):
        ...
        self._add_vehicle(parsed_message["id"], spawn_point, parsed_message["blueprint"], parsed_message.get("color", self.DEFAULT_VEHICLE_COLOR), verbose=False)

        print(f"[!] Reloaded vehicle with ID = {parsed_message['id']} with blueprint = {parsed_message['blueprint']} and color = {parsed_message.get('color', self.DEFAULT_VEHICLE_COLOR)}")
